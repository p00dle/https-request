...
describe('CookieJar', () => {
  ...
  test('addCookie overwrites with same key, domain, and path else adds', () => {
    ...
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'b', domain: 'abc.com' }));
    expect(jar.toJSON()).toHaveLength(1);
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'c', domain: 'abc.com' }));
    expect(jar.toJSON()[0].value).toBe('c');
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'e', domain: 'abc.com', path: '/foo' }));
    expect(jar.toJSON()).toHaveLength(2);
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'g', domain: 'abc.com', path: '/foo' }));
    expect(jar.toJSON()).toHaveLength(2);
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'g', domain: 'bob.com', path: '/foo' }));
    expect(jar.toJSON()).toHaveLength(3);
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'g', domain: 'bob.com' }));
    expect(jar.toJSON()).toHaveLength(4);
  });
  test('removeCookie removes cookie that matches key, domain or path', () => {
    const jar = new CookieJar();
    jar.addCookies([
      CookieJar.makeCookie({ key: 'a', value: 'b', domain: 'abc.com' }),
      CookieJar.makeCookie({ key: 'a', value: 'e', domain: 'abc.com', path: '/foo' }),
      CookieJar.makeCookie({ key: 'a', value: 'g', domain: 'bob.com', path: '/foo' }),
      CookieJar.makeCookie({ key: 'a', value: 'h', domain: 'bob.com' }),
    ]);
  ...
  test('getCookie gets the first cookie that matches key, or optionally domain and path', () => {
    ...
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'e', domain: 'abc.com', path: '/foo' }));
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'g', domain: 'bob.com', path: '/foo' }));
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: 'h', domain: 'bob.com' }));
  ...
  test('cookies are removed when they are past their expiry time when getRequestCookies is called', () => {
    const jar = new CookieJar();
    jar.addCookie(CookieJar.makeCookie({ key: 'a', value: '', domain: 'example.com', expires: Date.now() + 10_000 }));
